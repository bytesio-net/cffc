...
export const init = (): BuildConfig => {
  ...
  // Extract custom --export flag (inverse of noExport)
  const exportIndex = args.indexOf('--export');
  const noExport = exportIndex === -1; // Default to true if --export not present
  if (exportIndex !== -1) args.splice(exportIndex, 1);
...
const transpileFile = (
  ...
  const result = ts.transpileModule(sourceFile.text, {
    compilerOptions: {
      ...options,
      module: noExport ? ts.ModuleKind.ESNext : options.module,
    },
